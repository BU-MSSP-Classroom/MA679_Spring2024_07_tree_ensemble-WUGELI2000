...
set.seed(1)
nrow(Carseats)
train = sample(seq(400), 300, replace = FALSE)
...
library(tree)
tree.model <- tree(Sales ~ ., data = car.train)

plot(tree.model)
text(tree.model, pretty = 0)

test.predictions <- predict(tree.model, car.test)

test.mse <- mean((car.test$Sales - test.predictions)^2)
test.mse
```

(c) Use cross-validation in order to determine the optimal level of tree complexity. Does pruning the tree improve the test MSE?
```{r, fig.width=5, fig.height=3}
tree_model <- tree(Sales ~ ., data = car.train)
...
pruned_test_predictions <- predict(pruned_tree, car.test)

pruned_test_mse <- mean((car.test$Sales - pruned_test_predictions)^2)
pruned_test_mse
```
...
bagging_model <- randomForest(Sales ~ ., data = car.train, mtry = ncol(car.train)-1, importance = TRUE)
bagging_predictions <- predict(bagging_model, car.test)

bagging_test_mse <- mean((car.test$Sales - bagging_predictions)^2)
bagging_test_mse
...
rf_model <- randomForest(Sales ~ ., data = car.train, importance = TRUE)
rf_predictions <- predict(rf_model, car.test)

rf_test_mse <- mean((car.test$Sales - rf_predictions)^2)
rf_test_mse
...
```{r}
library(dbarts)
bart_model <- bart(Sales ~ ., data = car.train)
importance <- get_variable_importance(bart_model)
predictions <- predict(bart_model, new_data = car.test)
mse <- mean((car.test$Sales - predictions)^2)
print(importance)
print(mse)
```
...
Your code:
```{r,echo=TRUE}
OJ_test_predictions <- predict(tree.OJ, newdata = OJ.test, type = "class")

confusion_matrix <- table(OJ.test$Purchase, OJ_test_predictions)
...
```{r,echo=TRUE}
optimal_size <- which.min(cv_tree$dev)
best_tree_size <- cv_tree$size[optimal_size]
best_tree_size
```
...
optimal_size <- which.min(cv_tree$dev)
best_size <- cv_tree$size[optimal_size]

pruned_tree <- prune.tree(tree_model, best = best_size)

if (best_size == max(cv_tree$size)) {
...
Your code:
```{r,echo=TRUE}
unpruned_train_predictions <- predict(tree_model, OJ.train, type = "class")

unpruned_train_error <- mean(unpruned_train_predictions != OJ.train$Purchase)

pruned_train_predictions <- predict(pruned_tree, OJ.train, type = "class")
pruned_train_error <- mean(pruned_train_predictions != OJ.train$Purchase)

print(paste("Unpruned tree error:", unpruned_train_error))
print(paste("Pruned tree error:", pruned_train_error))

if (unpruned_train_error > pruned_train_error) {
  print("The unpruned tree has a higher training error rate.")
} else if (unpruned_train_error < pruned_train_error) {
  print("The pruned tree has a higher training error rate.")
} else {
  print("The training error rates are equal.")
}
```
